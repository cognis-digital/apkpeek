...
[![PyPI](https://img.shields.io/pypi/v/cognis-apkpeek.svg)](https://pypi.org/project/cognis-apkpeek/)
[![CI](https://github.com/cognis-digital/apkpeek/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/apkpeek/actions)
...
APKPEEK exists for one job — one-command static triage of android apk/aab binaries: surfaces hardcoded secrets, exported components, dangerous permissions, and insecure manifest flags as a single sarif report. — and does it without a SaaS bill or heavyweight setup.
...
```bash
apkpeek --version
apkpeek scan .                      # scan the current project
apkpeek scan . --format json
apkpeek scan . --fail-on high       # non-zero exit for CI gates
apkpeek mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **MobSF (Mobile Security Framework) static analyzer, minus the heavyweight web UI**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`apkpeek` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
